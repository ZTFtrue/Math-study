...
#        机器学习Support Vector Machines(svm)
  ...
  其它
    ...
    常见进制换算
      二进制 转 十进制
      @ 二进制数从低位到高位（即从右往左）计算，每一位以2为底，以位数为幂的结果乘以所在位数的值，然后相加。例如：
      @ (101011)B 转十进制
      @1. 第0位 1 x 2^0 = 1；
      @2. 第1位 1 x 2^1 = 2；
      @3. 第2位 0 x 2^2 = 0；
      ...
      八进制 转 十进制
      @ 基本原理同二进制转十进制，从低位到高位（即从右往左）计算，每一位以8为底，以位数为幂的结果乘以所在位数的值，然后相加。
      @将八进制的(53)O转换为十进制的步骤如下：
      @1. 第0位 3 x 8^0 = 3；
      @2. 第1位 5 x 8^1 = 40；
      @3. 3+40=43，即(53)O=(43)D。
      十六进制 转 十进制
      @ 基本原理依然同二进制转十进制，只是需要注意响应的ABCDEF与十进制对应于10、11、....、15 . 