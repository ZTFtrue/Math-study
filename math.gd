...
Math
  ...
  导数与微分
    ...
    曲率
    洛必达法则
      @ 洛必达法则  设函数f(x),g(x)满足下列条件  1. limf(x)=0,limg(x)=0(或limf(x)=∞，limf(g)=∞);  2. f(x),g(x) 可导，且g'(x)≠0；  3. lim{f'(x)/g'(x)}存在或为∞。  4. 则lim{f(x)/g(x)}=lim{f'(x)/g'(x)}
    ...
    微分
      ...
      微分的几何意义
        ...
        @ 当$\mid\Delta x\mid$ 很小时，也就是说，在点$M_0$的附近，可以用切线的纵坐标的增量$dy$来近似替代曲线纵坐标的改变量$\Delta y$,即$\mid\Delta y\thickapprox dy\mid$.
